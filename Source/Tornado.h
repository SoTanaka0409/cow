...
private:
	...
	// 成長弾などオブジェクトのサイズ変化に伴い、物理的な衝突範囲を即座に同期する。
	// [入出力:なし] オブジェクトのスケール増減およびコライダー半径の再計算を行う。
	void UpdateScaleAndRadius();

	// プレイヤーの現在座標を常に追従させ、回避行動を強要するゲームプレイを補強する。
	// [入出力:なし] プレイヤーへ向けた移動ベクトルの補間および速度の更新を行う。
	void UpdateHomingPlayer();

	// 壁などの地形を活用した反射攻撃を実現し、閉所での戦闘に緊張感を与える。
	// [入出力:なし] 地形との衝突検知、および法線に基づく反射ベクトルの算出・適用を行う。
	void UpdateWallBounce();

	// オブジェクトの状態に応じた視覚・聴覚フィードバックを生成し、操作の手応えを提示する。
	// [入出力:なし] パーティクルエフェクトの発生要求、および環境音・効果音の再生を行う。
	void UpdateEffectAndSound();

	// 被弾時の吹き飛びを計算し、攻撃の重み（Weight）とキャラクターの硬直状態を表現する。
	// [入出力:なし] 速度ベクトルの強制加算、および摩擦による速度の減衰処理を行う。
