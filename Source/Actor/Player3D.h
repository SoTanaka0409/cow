...
/// @brief プレイヤーの入力処理、移動、状態遷移、ステータス計算を一元管理するクラス
/// @details 各種サブマネージャーを内包するため、ライフサイクルの管理（メモリリーク）に注意すること
...
public:
	...
	/// @brief チュートリアル中など、特定イベント進行前に吸い込みが暴発して進行不能になるのを防ぐための状態参照
	/// @details なし [出力] 吸い込みフラグの真偽値 [副作用] なし
...
public:
	...
	/// @brief カメラの向きに依存するため、必ずカメラのUpdateより後に実行して進行方向のズレを防ぐこと
	/// @details なし [出力] なし [副作用] プレイヤー座標の更新
	void MoveEx();

	/// @brief 多重呼び出しによるゲージの異常消費を防ぐため、アクション制限状態（スタン等）では必ずガードをかけること
	/// @details なし [出力] なし [副作用] ゲージ消費と吸い込み状態の変更
	void Play();

	/// @brief モデル移動の後に呼ばないと、当たり判定が1フレーム取り残されて壁抜けバグの原因になるため順序厳守
	/// @details なし [出力] なし [副作用] コライダーの座標同期
	void ColliderUpdate();

	/// @brief 敵との押し出し判定等で画面外（奈落）へ落ちて進行不能になるバグを防ぐ最終防波堤
	/// @details なし [出力] なし [副作用] 座標のクランプ処理
	void ScreenOutCheck();

	/// @brief ジンバルロックや逆方向への急旋回時のカクつきを防ぐため、現在角から目標角へLerp補間して向かせること
	/// @details なし [出力] なし [副作用] モデル回転角の更新
	void RotationByMove();

	/// @brief プレイヤーのライフサイクルと紐づく機能（Skill等）の更新を一元化し、外部マネージャからの呼び出し忘れを防ぐ
	/// @details なし [出力] なし [副作用] 内部保持する各サブマネージャーの更新
	void ManagerUpdate();
	...
	/// @brief スキルバフやレベル補正が乗算されるため、計算漏れバグを防ぐべく元ステータスは直接参照せず必ず本関数を通すこと
	/// @param id 対象のステータスID [出力] 最終計算値 [副作用] なし
	float Status(StatusID id);

	/// @brief 値を設定する
	/// @param f 設定する速度 [出力] なし [副作用] 攻撃速度の更新
	...
	/// @param check 相手のコライダー [出力] なし [副作用] 衝突イベントの発火
	virtual void OnEnter(Collider* collider, Collider* check) override;
	virtual void OnTrigger(Collider* collider, Collider* check) override;
	virtual void OnExit(Collider* collider, Collider* check) override;

	/// @brief なし [出力] カプセルコライダーのポインタ [副作用] なし
	CapsuleCollider* GetCollisionCollider() { return capsule_collider_; }

	/// @brief 当たり判定と見た目の乖離を防ぐため、必ずコライダーの半径も同期して変更する実装にすること
	/// @param scale 倍率 [出力] なし [副作用] モデルスケールと判定半径の変更
	void SetScale(float scale);

	/// @brief 行動不能時に吸い込みや移動が暴発しないよう、Update内の各アクション処理でこのフラグを監視すること
	/// @param stunTime スタン時間(フレーム) [出力] なし [副作用] スタン状態のフラグ設定
	void ApplyStun(int stunTime);

	/// @brief 毎フレーム呼ばれるとVRAM枯渇と処理落ちを引き起こすため、スキル発動の瞬間（エッジ）のみ発火させること
	/// @details なし [出力] なし [副作用] エフェクトの生成と再生
