...
/// @brief unique_ptrによるオブジェクトの自動破棄
ObjectManager::~ObjectManager()
{
	// 手動deleteによるメモリリークや二重解放バグを防ぐため、解放処理はスマートポインタ(unique_ptr)に委譲する
}

/// @brief 全オブジェクトの更新、カメラ距離の計算、および不要オブジェクトの削除
...
/// @brief データの追加処理を行う
/// @details object3D (追加する3Dオブジェクトのポインタ)
/// @details 管理リストへの追加およびタグキャッシュへの登録
...
/// @brief データの除外処理を行う
/// @details object3D (管理から除外する3Dオブジェクトのポインタ)
/// @details unique_ptrの所有権放棄、リストからの除外、キャッシュの再構築
...
/// @brief 削除フラグの立った3Dオブジェクトの破棄とキャッシュ再構築
...
/// @brief 値を取得する
/// @return 条件に合致した最初のObject3Dポインタ（無ければnullptr）
/// @details tag (検索対象の3Dタグ)
...
/// @brief 値を取得する
/// @return 条件に合致したObject3Dポインタのリスト参照
/// @details tag (検索対象の3Dタグ)
...
/// @brief データの追加処理を行う
/// @details object2D (追加する2Dオブジェクトのポインタ)
/// @details 管理リストへの追加およびタグキャッシュへの登録
...
/// @brief 削除フラグの立った2Dオブジェクトの破棄とキャッシュ再構築
...
/// @brief 値を取得する
/// @return 条件に合致した最初のObject2Dポインタ（無ければnullptr）
/// @details tag (検索対象の2Dタグ)
...
/// @brief 値を取得する
/// @return 条件に合致したObject2Dポインタのリスト参照
/// @details tag (検索対象の2Dタグ)
