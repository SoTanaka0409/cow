 ...
 * [入力] filename: モチE��ファイル, pos: 出現基準座樁E scale: 拡大玁E tag: 牛�Eタグ, count: 生�E数, is_fever: フィーバ�Eフラグ
 * [出力] なぁE
...
void CowManager::SpawnCow(std::string filename, VECTOR pos, float scale, CowMove::TagCow tag, int count, bool is_fever)
{
	for (int i = 0; i < count; i++)
	{
		...
		if (mCreatures.size() >= 30)
		{
			...
			{
				// 金�E牛を確実に出現させるため、�Eレイヤーから最も遠ぁE��通�E牛を優先して破棁E��枠を空ける
				bool erased = false;
				...
				{
					// 全ての牛が画面冁E��どの場合、一番古ぁE��のの削除フラグを立ててリストから除外すめE
